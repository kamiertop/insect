...
def parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(description="Evaluate insect classifier on test set")
	parser.add_argument("--config", type=str, default="./config.toml", help="Path to config.toml")
	parser.add_argument("--checkpoint", type=str, default="", help="Checkpoint path, default is <train.save_dir>/best.pt")
	parser.add_argument("--batch-size", type=int, default=0, help="Override eval batch size, 0 means use train.batch_size")
	parser.add_argument("--num-workers", type=int, default=-1, help="Override num_workers, -1 means use train.num_workers")
	parser.add_argument("--max-steps", type=int, default=0, help="0 means evaluate full test set")
	parser.add_argument("--save-dir", type=str, default="./logs/eval", help="Directory to save evaluation csv files")
...
def build_metric_tables(confusion: torch.Tensor, idx_to_class: dict[int, str]) -> tuple[dict[str, float], list[dict[str, float]], list[list[int]]]:
...
def main() -> None:
	...
	checkpoint_path = Path(args.checkpoint) if args.checkpoint else (Path(cfg.train.save_dir) / "best.pt")
	ckpt, class_to_idx = load_checkpoint(checkpoint_path, device)
	idx_to_class = {v: k for k, v in class_to_idx.items()}

	test_ds = InsectDataset(config=cfg, split="test", transform=build_eval_transform())
	align_test_mapping(test_ds, class_to_idx)

	batch_size = args.batch_size if args.batch_size > 0 else cfg.train.batch_size
	num_workers = args.num_workers if args.num_workers >= 0 else cfg.train.num_workers
